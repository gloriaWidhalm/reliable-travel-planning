...
query = """ CREATE TABLE IF NOT EXISTS services AS SELECT * FROM './data/*.csv'"""
...
df_product_id_counts["delay_percentage"] = (
    df_product_id_counts["delay_count"] / df_product_id_counts["total_count"]
) * 100  # add percentage
...
columns_str = ", ".join(
    column_names
)  # Create the column list for PARTITION BY dynamically
...
largest_delay_lines_df = pd.DataFrame(
    ten_largest_delay_lines
...
delay_stats = (
    operator_delay_data.groupby("OPERATOR_ID")["delay_seconds"]
    .agg(average_delay="mean", median_delay="median")
    .reset_index()
)
...
for _, row in top_5_median_delay.iterrows():
    print(
        f"Operator ID: {row['OPERATOR_ID']}, Median Delay: {row['median_delay']} seconds"
    )
...
for _, row in top_5_average_delay.iterrows():
    print(
        f"Operator ID: {row['OPERATOR_ID']}, Average Delay: {row['average_delay']} seconds"
    )

# TIME SERIES ANALYSIS
# Convert ARRIVAL_TIME to datetime and extract useful components
delay_data["ARRIVAL_TIME"] = pd.to_datetime(
    delay_data["ARRIVAL_TIME"], format="%d.%m.%Y %H:%M"
)
...
avg_delay_by_stop = (
    delay_data.groupby("STOP_NAME")["delay_seconds"].mean().sort_values(ascending=False)
)
...
IQR = Q3 - Q1
outliers = delay_data[
    (delay_data["delay_seconds"] < (Q1 - 1.5 * IQR))
    | (delay_data["delay_seconds"] > (Q3 + 1.5 * IQR))
]
